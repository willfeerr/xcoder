...
async function record(params, timeoutMs) {
    const id = randomUUID();
    ...
    return new Promise((resolve, reject) => {
        ...
        child.once("exit", (code, signal) => {
            if (code === 0 || signal === "SIGTERM")
                return;
            clearTimeout(timer);
            reject(new Error(`Worker de vídeo encerrou com code=${String(code)} signal=${String(signal)}${stderr.trim() ? `: ${stderr.trim()}` : ""}`));
        });
        child.send({ id, method: "record", params });
    });
}
export const browserRecordTool = {
    name: "browser_record",
    description: "Grava vídeo WebM e trace de uma página animada, com rolagem automática e diagnóstico de console/rede.",
    risk: "execute",
    inputSchema: {
        type: "object",
        properties: {
...
//# sourceMappingURL=browser-record-tool.js.map